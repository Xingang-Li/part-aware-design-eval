...
class SDM_NET():
    ...
    gpu_opt = tf.GPUOptions(allocator_type = 'BFC', allow_growth = True) #there was no per_process
...
#-------------------------------------------------------------network framework---------------------------------------------------------------------------------------
    ...
    def __init__(self, datainfo):
        ...
        if self.union: #default 0
            ...
            self.structure_feature = tf.reshape(tf.concat([hidden_code, inputs_symmetry], axis = 2), [tf.shape(self.encode)[1], -1])
        else:

            hidden_code = tf.transpose(self.test_encode, perm = [1, 0, 2])
            print(np.shape(hidden_code))
            self.structure_feature = tf.reshape(tf.concat([hidden_code, inputs_symmetry], axis = 2), [tf.shape(self.test_encode)[1], -1]) #obtain features for SPVAE

        output_vae = self.build_vae_block_for_structure(self.structure_feature, self.hiddendim[1], name = 'vae_block_structure') #for SPVAE
...
#------------------------------------------------------------applications--------------------------------------------------------------------------------------
    ...
    def obtain_embeddings_total(self, datainfo, epoch = 0): #this function cannot be run due to the limitation of the GPU memory, used seperate functions for the same purpose
        with tf.Session(config = self.config) as sess:
            tf.global_variables_initializer().run()
            if epoch == 0:
                _success, epoch = self.load(sess, self.checkpoint_dir_structure)
    ...
    def obtain_partVAE_embeddings(self, datainfo, epoch = 0): #obtain partVAE embeddings first
        with tf.Session(config = self.config) as sess:
            tf.global_variables_initializer().run()
            if epoch == 0:
                _success, epoch = self.load(sess, self.checkpoint_dir_structure)
    ...
    def obtain_SPVAE_embeddings(self, datainfo, epoch = 0): #obtain SPVAE embeddings from partVAE embeddings
        with tf.Session(config = self.config) as sess:
            tf.global_variables_initializer().run()
            if epoch == 0:
                _success, epoch = self.load(sess, self.checkpoint_dir_structure)
            ...
            path = self.checkpoint_dir_structure +'/../recon'+str(epoch) #must exist

            for i in range(self.part_num):
                path_mat = path + '/' + self.part_name[i] + '/recover.mat'
                # data = h5py.File(path_mat, mode='r')
                data = sio.loadmat(path_mat)
                datalist = data.keys()
                emb_PartVAE = np.array(data['emb']).astype('float32')
                self.test_encode.append(emb_PartVAE)

            random = np.zeros((len(self.feature), 200)).astype('float32')
            dictbase = {self.embedding_inputs: np.array(self.test_encode).astype('float32'), self.inputs_symmetry: self.symmetry_feature}
            dictrand = {x: random[:, 0: np.shape(x)[1]] for x in self.embedding_inputs}
            feed_dict = merge_two_dicts(dictbase, dictrand)

            inputs_structure = sess.run(self.structure_feature, feed_dict = feed_dict)

            output_vae = self.build_vae_block_for_structure(inputs_structure, self.hiddendim[1], name='vae_block_structure')  # for SPVAE

            self.post_output_vae(output_vae)  # for SPVAE

            emb_SPVAE = self.test_encode[self.part_num]

            sio.savemat(path + '/recover_SPVAE.mat', {'tid': self.train_id, 'vid': self.valid_id, 'emb_SPVAE': emb_SPVAE})
