...
#------------------------------------------------------------applications--------------------------------------------------------------------------------------
    ...
    def obtain_embeddings_total(self, datainfo, epoch = 0): #this function cannot be run due to the limitation of the GPU memory, used seperate functions for the same purpose
        with tf.Session(config = self.config) as sess:
            ...
            for i in range(self.part_num + 1):
                embeddings_list = []
                for bidx in range(len(datainfo.modelname)//self.batch_size + 1):
                    id_feature = self.feature[bidx * self.batch_size : min((bidx + 1) * self.batch_size, len(datainfo.modelname)), :, :, :]
                    id_symmetry_feature = self.symmetry_feature[bidx * self.batch_size : min((bidx + 1) * self.batch_size, len(datainfo.modelname))]
                    # embeddings = sess.run([self.encode[i]], feed_dict = {self.inputs_feature: id_feature, self.inputs_symmetry: id_symmetry_feature})[0] #only one needs to be run
                    embeddings = sess.run(self.encode[i], feed_dict={self.inputs_feature: id_feature,
                                                                       self.inputs_symmetry: id_symmetry_feature})
                ...
                if i < self.part_num:
                    ...
                    sio.savemat(path + '/' + self.part_name[i]+'/recover.mat', {'model_names': datainfo.modelname, 'emb':embeddings_list})

                else:
                    sio.savemat(path+'/recover_sym.mat', {'model_names': datainfo.modelname, 'emb':embeddings_list})
